...
- 正式版 GUI 的离线动作统一进入 `.live-note/runtime-v2/control.db` 控制平面，由 SQLite 任务表按严格 FIFO 串行执行；关闭应用后，未开始的排队任务会在下次启动时恢复
- 历史页里的“取消所选”现在同时支持两类导入任务：尚未开始的排队导入，以及当前正在运行的导入；运行中的导入会发送取消请求并在最近的安全边界停止
- 如果应用上次退出时某个离线任务仍处于运行中，它会在下次启动时被标记为“中断”，并从 SQLite 待读任务里移除，不会自动重跑
