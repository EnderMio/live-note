...
from live_note.runtime.session_mutations import apply_workspace_session_command
...
def publish_failure_outputs(
    ...
    *,
    ...
    failed_metadata = apply_workspace_session_command(
...
def publish_final_outputs(
    ...
    *,
    ...
    completed_metadata = apply_workspace_session_command(
        workspace_root_dir(workspace.root),
        workspace,
        SessionCommandKind.COMPLETE.value,
        payload={"display_status": structured_status},
    )
    workspace.write_session(completed_metadata)
